...
# only for tamu
    ...
    @classmethod
    def energy_correction(cls, energy):
        # this method is for fixing usingt he wrong mpl.init file during
        # the TRACER project
        return ((energy+0.119861)/0.002843)*0.005282909-1.327800034

    def fix_energy_write_mpl(self, output_dir, filename):
        # this method is for fixing usingt he wrong mpl.init file during
        # the TRACER project. After correcting for the energy, write data
        # back to a binary mpl file
        output_path = os.path.join(output_dir, filename)
        new_energy = np.copy(self.energy_correction(self.energy_monitor) * 1000).astype(int)
        new_dict = self.data_dict.copy()
        new_dict['energy_monitor'] = new_energy.tolist()
        list_size = len(new_dict['energy_monitor'])
